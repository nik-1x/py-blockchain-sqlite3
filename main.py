...
db = sqlite3.connect(appconfig["db"])
cur = db.cursor()
...
initBlockchain()
addBlock("user_1", "vote_FIRST", 1)
addBlock("user_2", "vote_SECOND", 1)
addBlock("user_3", "vote_SECOND", 1)
addBlock("user_4", "vote_FIRST", 1)
addBlock("user_5", "vote_SECOND", 1)

for x in getBlocks():
    print(x, "block is", "approved" if validate_block(x[0]) else "not_approved")
